...
def selectArea(city,choose,cookie,userAgent):
    ...
    for i in range(0, len(name_list)):
        print(i, name_list[i])
    num = simpledialog.askinteger("输入地区编号", "请输入所需要爬取的地区的编号")
    print(url_list[num])
    return url_list[num]
...
def main():
    ...
    def start_spider():
        ...
        area_url = selectArea(city, choose, cookie, user_agent)
        end_page = int(fileMaxPage(area_url, cookie, user_agent))

        for i in range(start_page, end_page + 1):
            log_text.insert(tk.END, f"开始爬取第 {i} 页...\n")
            df_file = requestMeiTuan(area_url, i, cookie, user_agent)
            saveExcel(file_name, df_file)
            log_text.insert(tk.END, f"第 {i} 页完成！\n")

    city_label = tk.Label(root, text="请输入城市（拼音）:")
    city_label.pack()
    city_entry = tk.Entry(root)
    city_entry.pack()

    file_name_label = tk.Label(root, text="请输入文件名:")
    file_name_label.pack()
    file_name_entry = tk.Entry(root)
    file_name_entry.pack()

    start_page_label = tk.Label(root, text="请输入开始页数:")
    start_page_label.pack()
    start_page_entry = tk.Entry(root)
    start_page_entry.pack()

    cookie_label = tk.Label(root, text="请输入cookie（留空使用默认值）:")
    cookie_label.pack()
    cookie_entry = tk.Entry(root)
    cookie_entry.pack()

    start_button = tk.Button(root, text="开始爬虫", command=start_spider)
    start_button.pack()
